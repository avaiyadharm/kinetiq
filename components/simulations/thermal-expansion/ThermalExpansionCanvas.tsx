...
export const ThermalExpansionCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    vizSettings, spatialStressProfile, time
  ]);
  ...
  const drawRod = (ctx: CanvasRenderingContext2D, W: number, H: number) => {
    ...
    // Draw rod in segments for thermal gradient coloring
    for (let i = 0; i < N - 1; i++) {
      const x0 = rodX + (i / (N - 1)) * visualW;
      const x1 = rodX + ((i + 1) / (N - 1)) * visualW;
      const segW = x1 - x0;
      ...
      if (vizSettings.showThermalGradient) {
        ...
        grad.addColorStop(0, thermalColor(thermalProfile[i], 0.85));
        grad.addColorStop(1, thermalColor(thermalProfile[i + 1], 0.85));
        ctx.fillStyle = grad;
      ...
      if (vizSettings.showStressColors && spatialStressProfile[i] !== 0) {
        ...
        ctx.fillStyle = stressColor(spatialStressProfile[i], σ_y, 0.4);
        ctx.fillRect(x0, rodY, segW, rodH);
      }
    }

    // Base fill if no gradient
    if (!vizSettings.showThermalGradient) {
      ctx.fillStyle = "#222228";
      ctx.fillRect(rodX, rodY, visualW, rodH);
    }

    // Rod border
    ctx.strokeStyle = isFailed ? "rgba(239,68,68,0.8)" : "rgba(255,255,255,0.18)";
    ctx.lineWidth = isFailed ? 2.5 : 1.5;
    ctx.strokeRect(rodX, rodY, visualW, rodH);

    // Constraint anchors
    drawRodAnchors(ctx, rodX, rodY, visualW, rodH, baseW, W, H);

    // Cracks
    if (isFailed && crackLocations.length > 0) {
      drawCracks(ctx, rodX, rodY, visualW, rodH);
    }

    // Dimension arrows (show real expansion)
    drawDimensionArrows(ctx, rodX, rodY, baseW, visualW, rodH, W);

    // Heat front indicator
    if (vizSettings.showHeatFront && constraint !== "fixed") {
      drawHeatFront(ctx, rodX, rodY, visualW, rodH);
    }
  };
  ...
  const drawRailway = (ctx: CanvasRenderingContext2D, W: number, H: number) => {
    ...
    const buckleAmp = willBuckle
      ? 38 * vizSettings.deformationExaggeration * Math.abs(realDeltaL * 5)
      : 0;
  ...
  const drawDimensionArrows = (
    ...
    // Reference line (original length)
    const refX = rodX; // For rod expanded from center, adjust:
    ...
    if (visualW > baseW) {
      ...
      ctx.fillText(
        `ΔL (real) = ${realMM >= 1 ? realMM.toFixed(1) + "mm" : (realDeltaL * 1000).toFixed(3) + "mm"}  ×${mag} magnified`,
  ...
  const drawAtomicLattice = (ctx: CanvasRenderingContext2D, W: number, H: number) => {
    ...
    // Bond stiffness (GPa range, scaled for visualization)
    const bondK = (mat.youngsModulus / 1e11) * 15;
    const A = PhysicsEngine.vibrationAmplitude(avgTemperature, Math.max(bondK, 0.5));
    // Scale to pixels
    const ampPx = Math.min(6, A * 2e12);

    // Anharmonic mean position shift (atoms shift right at high T)
    const shift = PhysicsEngine.anharmonicShift(avgTemperature, mat.alpha0) * 3000;
    const spacing = 22 + shift;

    const cols = 5, rows = 4;
    const startX = cx - (cols - 1) * spacing / 2;
    const startY = cy - (rows - 1) * spacing / 2;

    // Phase for time-based vibration
    const phase = time * 8;
    ...
    for (let r = 0; r < rows; r++) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        // Anharmonic: positive bias on displacement
        const dx = ampPx * Math.sin(phase + phaseShift);
        // Asymmetric: atom spends more time at r > r0 (positive side)
        const asymmetry = ampPx * 0.3 * Math.sin(2 * (phase + phaseShift)); // second harmonic
        const dy = ampPx * Math.cos(phase * 1.1 + phaseShift);
        atomPos[r][c] = {
          x: startX + c * spacing + dx + asymmetry + shift * 0.2 * c,
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        const pos = atomPos[r][c];
        // Vibration envelope (anharmonic asymmetry: wider right side)
        ctx.save();
        ctx.globalAlpha = 0.12;
        ctx.fillStyle = thermalColor(avgTemperature, 1);
        ctx.beginPath();
        ctx.ellipse(pos.x + ampPx * 0.15, pos.y, ampPx * 1.4, ampPx * 1.0, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
    ...
    ctx.fillRect(cx - 42, cy + radius - 16, 84, 14);
    ctx.fillStyle = thermalColor(avgTemperature, 0.9);
    ctx.font = "bold 6.5px monospace";
    ctx.textAlign = "center";
    ctx.fillText(`LATTICE  A = ${(ampPx * 0.5e-12).toExponential(1)} Å`, cx, cy + radius - 5);

    // Anharmonic note
    ctx.fillStyle = "rgba(255,255,255,0.3)";
    ctx.font = "5.5px monospace";
    ctx.fillText("anharmonic: ⟨r⟩ > r₀ as T rises", cx, cy - radius + 12);
  };
